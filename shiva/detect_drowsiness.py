...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if status1 == 2 and status2 == 2:
            ...
            count += 1
            text = "Eyes Closed, Frame count: " + str(count)
            (text_width, text_height), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_COMPLEX, 1, 2)
            # Create a red rectangle based on the text size
            cv2.rectangle(frame, (10, 10), (10 + text_width + 10, 10 + text_height + 10), (255,255, 255), -1) 
            # Put the text on the white rectangle
            cv2.putText(frame, text, (20, 40), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 0), 2)
            ...
            if count >= 10:
                cv2.putText(frame, "Drowsiness Alert!!!", (100, height-20), cv2.FONT_HERSHEY_COMPLEX, 1.5, (0, 0, 255), 2)
                if not alarm_on:
                    alarm_on = True
            ...
            text="Eyes Open"
            (text_width, text_height), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_COMPLEX, 1, 2)
            # Create a white rectangle based on the text size
            cv2.rectangle(frame, (10, 10), (10 + text_width + 10, 10 + text_height + 10), (255, 255, 255), -1)
            # Put the text on the white rectangle
            cv2.putText(frame, text, (20, 40), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0, 0), 2)
            #cv2.putText(frame, "Eyes Open", (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
            count = 0
            alarm_on = False
